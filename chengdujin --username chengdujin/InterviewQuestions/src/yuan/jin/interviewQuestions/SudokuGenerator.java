...
/**
 * Sudoku generator Generates a Sudoku grid of the size passed as parameter.
 * First call the constructor with the size of the region. Then call generate()
 * with a boolean if you want trace or not.
 * 
 * produce a Sudoku grid
 * 
 * http://www.dreamincode.net/code/snippet3447.htm
 * 
...
public class SudokuGenerator {
	...
	SudokuGenerator(int size) {
		...
		ran = new Random();
		// arraylist that will contain the possible values for every case in the
		// grid
	...
	public void generate(boolean traceOn) {
		...
		while (currentRow < size) {
			...
			if (genRow(currentRow)) {
				if (traceOn) {
					System.out.print("Row " + (currentRow + 1)
							+ " generated after " + trials[currentRow]
							+ " trial");
			...
			// so despite all our effort it does not fit we will have to restart
			// for the whole
			// row regions
			if (traceOn)
			...
			trials[currentRow] = 0;
			if (traceOn)
				System.out.println(". Starting over with row: "
						+ (currentRow + 1) + ".");
		}
	...
	// (it might be impossible to fill the row for example if the only element
	// left in the row
	// is 5 so the last column must be 5 but I may already have a 5 in the
	// column or region)
	...
	private int fillArrayList(int row, int col) {
		...
		// the region the columns to the left of my position have already been
	...
	// return the first and last row/column of the region into which is located
	// the (row or col)
	...
	public String toString() {
		...
		for (int i = 0; i < size; i++) { // for every row
			for (int j = 0; j < size; j++) { // and column
				...
				buffer.append(' ');
				buffer.append(Integer.toHexString((grid[i][j])).toUpperCase()); // add
																				// value
																				// to
																				// String
